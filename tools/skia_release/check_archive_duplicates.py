...
def ar(*args):
  return subprocess.check_output(['ar', *map(str, args)])
...
def skiko_archives(out_dir, target, build_type):
  ...
  if target in ('macos', 'ios', 'iosSim', 'tvos', 'tvosSim', 'linux'):
    ...
    libraries += [
        'raw_ptr', 'allocator_core', 'allocator_base', 'skia_ganesh_ext',
        'd3d12allocator', 'skia_graphite_ext', 'skottie', 'sksg', 'jsonreader',
    ]
    if build_type == 'Debug':
